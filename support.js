...
/* ── Offline-export: interne links laten werken ─────────────────────────────
   Deze site is geëxporteerd naar een mappenstructuur. Links die in de HTML
   staan zijn bij het exporteren al omgeschreven, maar links die React tijdens
   het renderen zet (sectorpaneel, klantcaserijen) blijven absoluut: "/sectoren
   /horeca". Die zijn niet statisch te vinden, dus vangen we ze bij de klik.

   De diepte van de huidige pagina leiden we af uit de stylesheet-verwijzing,
   die op elke pagina staat en al het juiste aantal ../ draagt. */
(function __offlineLinks() {
  if (location.protocol === 'http:' || location.protocol === 'https:') {
    // Op een echte server werken absolute paden gewoon; niets doen.
    if (location.hostname && location.hostname !== 'localhost' && location.hostname !== '127.0.0.1') return;
  }
  /* De diepte komt uit de support.js-verwijzing zelf: die staat op élke
     pagina, ook op de homepage (die zijn stijl inline draagt en dus geen
     ed-site.css laadt). */
  var eigen = document.querySelector('script[src$="support.js"]');
  var basis = eigen ? eigen.getAttribute('src').replace(/support\.js$/, '').replace(/^\.\//, '') : '';

  function naarBestand(href) {
    var hash = '';
    var i = href.indexOf('#');
    if (i > -1) { hash = href.slice(i); href = href.slice(0, i); }
    var pad = href.replace(/^\/+|\/+$/g, '');
    return basis + (pad ? pad + '/index.html' : 'index.html') + hash;
  }
  ...
  document.addEventListener('click', function (e) {
    ...
    e.preventDefault();
    location.href = naarBestand(href);
  }, true);
})();
